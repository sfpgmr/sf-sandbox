...
rad_range = np.arange(0., math.pi, math.pi / 64.)
...
for fid in fids.values() :
  ...
  if(len(coords) > 2) :
    try :
      ...
      mp = geomery.MultiPoint(coords).covex_hull
  else :
  ...
  def simplize_1() :
    ...
    for pt in translated_rect.exterior.coords :
      ...
      intersects = target.intersection(lst)
    ...
    return shrinked_rect
 
  try :
    simplized_rect = simplize_1()
    if((simplized_rect.area / mp.area) < 0.2) :
      unsuitable_results.append(coords)
      print(target_fid)
      try :
        result = None
        result_rad = 0.
        for rad in rad_range :
          rotated_polygon = affinity.rotate(mp,rad,'centroid',use_radians=True)
          answer = Polygon(get_maximal_rectangle(geometry.mapping(rotated_polygon)['coordinates'][0]))
          if(result == None) : 
            result = answer
            result_rad = rad
          else :
            if(answer.area > result.area) :
              result = answer 
              result_rad = rad
        ...
        if(result == None) :
          rect = simplize_1()
          rect = geometry.mapping(rect)['coordinates'][0]
        else :
          result = affinity.rotate(result,-result_rad,'centroid',use_radians=True)
          x1,y1 = result.centroid.xy
          x2,y2 = target.centroid.xy
          x1,y1 = x1[0],y1[0]
          x2,y2 = x2[0],y2[0]
          rect = affinity.translate(result,x2-x1,y2-y1)          
          rect = geometry.mapping(rect)['coordinates'][0]
...
with open(f'{work_dir}merged_simple.json',mode="w") as f:
  f.write(maps_json)
