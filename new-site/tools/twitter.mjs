...
export default class Twitter {
  ...
  async init() {
    ...
    this.db = sqlite.open("./data/tweets.db");
  }
  ...
  async getTweetThread(statusId) {
    ...
    this.db.exec("create table if not exists tweets (id text primary key ,value json,created int,updated int);");
    //await db.run("create table if not exists tweets (id int primary key ,value json,create_date int,update_date int);");

    let tweetData = [];
    const tweetStmt = db.prepare('select * from tweets where id = ?');
    const tweetReplaceStmt = db.prepare('replace into tweets(id,value,created,updated) values(?,?,?,?);');
    ...
    while (true) {
      ...
      if (!dbExists) {
        tweetReplaceStmt.run([tweet.id_str, JSON.stringify(tweet), date, date]);
      }
